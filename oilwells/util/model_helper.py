...
def get_latest_model(
    model_name: str, tag_name: str = None, tag_value: str = None, aml_workspace: Workspace = None
) -> AMLModel:
    """
    Retrieves and returns the latest model from the workspace
    ...
    try:
        ...
        if tag_name is not None and tag_value is not None:
            model_list = AMLModel.list(aml_workspace, name=model_name, tags=[[tag_name, tag_value]], latest=True)
        ...
        too_many_model_message = "Found more than one latest model. " f"Models found: {model_list}. " f"{tag_ext}"

        no_model_found_message = f"No Model found with name: {model_name}. " f"{tag_ext}"
